...
class JsonFichero:
    ...
    def leer(self):
        ...
        self.db_id = int(data["db_id"])
        self.pin = str(data["pin"])
        self.skid = str(data["skid"])
        self.modelo_ext = int(data["modelo"])
        self.modelo = settings.MODELOS[self.modelo_ext]
        self.path_base = os.path.join(os.path.dirname(os.path.dirname(os.getcwd())), str(data["path"]))
        self.datetime_aplicacion = data["datetime_aplicacion"]
        self.datetime_grabacion = data["datetime_grabacion"]
        self.cam1 = bool(data["cam1"])
        self.cam2 = bool(data["cam2"])

        if self.cam1:
            self.path_img_cam1 = os.path.join(self.path_base, f'c1_{data["pin"]}.jpg')
        if self.cam2:
            self.path_img_cam2 = os.path.join(self.path_base, f'c2_{data["pin"]}.jpg')
    ...
    def escribir(self):
        ...
        if self.res_cam1 is None or self.res_cam2 is None:
            raise ValueError("Faltan resultados en res_cam1 o res_cam2")
